...
int main()
{
	vector<int> v = { 1, 24, 6, 23, 4, 5 };
	
	int compareNum = 20;
	// 람다식
	// [캡쳐](매개변수)->(반환형식) {함수 정의부}
	// [캡쳐] : 람다 외부의 변수를 캡쳐 하는 용도
	// 20보다 큰 수의 개수
	...
	// 함수 포인터, (함수 객체함수, 람다) Callalbe 객체
	// Callable 객체를 매개변수, 넘기는 것을 '콜백 함수' => Delegate
	function<void(void)> fn; // fn 은 함수 포인터, 함수 객체, 람다
	fn = []() {};
...
#pragma region 실습
	...
	// players 배열을 정렬 player 한명이 갖고있는 pet의 데미지에 따라서 player를 정렬하라
