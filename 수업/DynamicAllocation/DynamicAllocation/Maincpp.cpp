...
public:
	...
	virtual void Attack() { cout << " Player 공격 " << endl; }
...
public:
	...
	virtual void Attack() override { cout << "Knight 공격 " << endl; }
...
// 가상함수테이블(vftable)
// : 가상함수들을 갖고있는 테이블.
// => 실행시간에 어떤 함수를 호출 해야되는지 알려준다.
// ==> 실행시간에 어떤타입인지 확인할 수 있는 수단.
...
int main()
{
	...
	Player*  k = new Knight(); // 업캐스팅 : 자식 클래스로 생성된 객체를 부모의 포인터자료형으로 가리키는 것
	(k)->Attack(); // => (Knight*) 없어도 중간에 타입이 무엇인지 확인 할수 있어야한다.
