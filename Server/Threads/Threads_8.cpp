...
int main()
{
	...
	// promise : 동기시점을 제공한다.(멀티쓰레드 시점에서)
	{
		// 미래에서 값을 설정 받을 변수
		std::promise<string> promise;
		std::future<string> future = promise.get_future(); // future에서 반환하겠다 라는 약속
	...
	{
		// task를 미래에 실행키셔 그 반환값을 f_result에 돌려주겠다.
	...
	// Cacluate  하는데 Event기반 Lock, SpinLock, SleepLock를 걸고 멀티쓰레드에서 작업하는 환경을 최소화
